...
import { assessRegions, createAgentStarterKit, createBicepPackage, deployPackage, deploymentKeysFor, getPackageDeploymentStatus, getPlatformResources, getUseCase, planArchitecture, previewDeployment, recommendModel as requestModelRecommendation, runDeploymentWhatIf, runPackageWhatIf, startDeployment, type ArchitecturePackageRequest, type ArchitecturePlan, type DeploymentManifest, type DeploymentProgress, type GeneratedAgentStarterKit, type GeneratedBicepPackage, type OperationalRequirements, type RegionAssessment, type UseCaseRecord } from './api'
...
type WorkspaceScreen = 'overview'|'definition'|'design'|'describe'|'data'|'connectors'|'channels'|'memory'|'requirements'|'model'|'region'|'architecture'|'iac'|'governance'|'provisioning'|'activity'|'ready'
...
const workspaceScreens=new Set<WorkspaceScreen>(['overview','definition','design','describe','data','connectors','channels','memory','requirements','model','region','architecture','iac','governance','provisioning','activity','ready'])
...
let activeUseCase:UseCaseRecord|undefined
...
const workspaceItems:{id:WorkspaceScreen;label:string;icon:string;status:string}[]=[
  ...
  {id:'definition',label:'Approved definition',icon:'✓',status:'complete'},
...
function WorkspaceNav({screen,resources,profile,navigate}:{screen:WorkspaceScreen;resources:Resource[];profile:string[];navigate:(screen:WorkspaceScreen)=>void}){
  ...
  return <aside className="workspace-nav"><div><span>{record.status.toUpperCase()} APPLICATION</span><h2>{record.name}</h2><small>{record.id} · {record.riskLevel} risk</small></div><nav>{workspaceItems.map(item=><button className={active===item.id?'active':''} disabled={item.status==='locked'} onClick={()=>navigate(item.id)} key={item.id}><i>{item.icon}</i><span>{item.label}</span><b className={item.status}>{item.status==='complete'?'✓':item.status==='action'?'!':item.status==='locked'?'🔒':'●'}</b></button>)}</nav><section className={`workspace-components ${showAzure?'azure-mode':'profile-mode'}`}><header><span>{showAzure?'AZURE COMPONENTS':'SOLUTION PROFILE'}</span><b>{showAzure?resources.length:profile.length}</b></header><p>{showAzure?'Generated from confirmed requirements.':'Business requirements collected so far.'}</p><div>{showAzure?resources.map(resource=><button onClick={()=>navigate('architecture')} key={resource.name}><i>{resource.icon}</i><span><strong>{resource.name}</strong><small>{resource.required?'Required baseline':resource.reason}</small></span></button>):profile.length?profile.map(item=><button onClick={()=>undefined} key={item}><i>✓</i><span><strong>{item}</strong><small>Confirmed requirement</small></span></button>):<span className="profile-empty">No requirements selected yet.</span>}</div></section></aside>
}
...
function DataKnowledge({resources,data,setData,next=()=>navigateWorkspace?.('connectors')}:{resources:Resource[];data:DataConfiguration;setData:React.Dispatch<React.SetStateAction<DataConfiguration>>;next?:()=>void}){
...
function ChannelsDistribution({resources,channels,setChannels,next=()=>navigateWorkspace?.('memory')}:{resources:Resource[];channels:ChannelConfiguration;setChannels:React.Dispatch<React.SetStateAction<ChannelConfiguration>>;next?:()=>void}){
...
function MemoryState({resources,memory,setMemory,next=()=>navigateWorkspace?.('requirements')}:{resources:Resource[];memory:string[];setMemory:React.Dispatch<React.SetStateAction<string[]>>;next?:()=>void}){
...
function ModelSelection({resources,recommendation,selected,setSelected,requiresEmbedding,selectedEmbedding,setSelectedEmbedding,embeddingCapacity,setEmbeddingCapacity,next=()=>navigateWorkspace?.('architecture')}:{resources:Resource[];recommendation:ModelRecommendation;selected:ModelId;setSelected:(model:ModelId)=>void;requiresEmbedding:boolean;selectedEmbedding:EmbeddingModelId|'';setSelectedEmbedding:(model:EmbeddingModelId)=>void;embeddingCapacity:number;setEmbeddingCapacity:(capacity:number)=>void;next?:()=>void}){
  ...
  return <WorkspacePage kicker="MODEL & CAPACITY" title="Choose the Foundry model deployment" description="The recommendation uses the confirmed application requirements. Compare approved alternatives and include the selected deployment in the generated package.">
    <div className="model-selection-layout">
      <section className="solution-card model-selection-card">
        ...
        <div className="model-options">{foundryModels.map(model=>{const isSelected=chosen.id===model.id;return <button className={isSelected?'selected':''} aria-pressed={isSelected} onClick={()=>setSelected(model.id)} key={model.id}><div><i>✦</i><span><strong>{model.name}</strong><small>{model.tier}</small></span>{model.id===recommendation.model.id&&<em>Recommended</em>}</div><p>{model.description}</p><dl><div><dt>Best for</dt><dd>{model.bestFor}</dd></div><div><dt>Latency</dt><dd>{model.latency}</dd></div><div><dt>Relative cost</dt><dd>{model.cost}</dd></div><div><dt>Context</dt><dd>{model.context}</dd></div></dl>{!isSelected&&<b>Select model</b>}</button>})}</div>
      ...
      <div className="model-side-rail"><Recommendation resources={resources}/></div>
    </div>
  </WorkspacePage>
}
...
function ProvisioningPage({resources,model=foundryModels[0],capacity='10K TPM'}:{resources:Resource[];model?:ModelOption;capacity?:string}){
...
function ApprovedDefinition({next}:{next:()=>void}){
  const record=activeUseCase
  if(!record)return null
  const approvedDate=record.approvedAt?new Intl.DateTimeFormat('en-US',{month:'short',day:'numeric',year:'numeric'}).format(new Date(record.approvedAt)):'Pending'
  return <WorkspacePage kicker="APPROVED SYSTEM OF RECORD" title="Approved use case definition" description="This information was approved before solution configuration and is inherited by every downstream design and deployment decision."><section className="approved-record-notice"><b>✓</b><span><strong>Approval baseline · {record.id}</strong><small>Read-only. Material changes require a governed amendment and re-review.</small></span><em>Approved {approvedDate}</em></section><div className="approved-definition-grid">{record.approvedDefinition.map((section,index)=><section className="solution-card approved-definition-section" key={section.key}><header><b>{String.fromCharCode(65+index)}</b><h2>{section.title}</h2><span>LOCKED</span></header><dl>{section.facts.map(fact=><div key={fact.label}><dt>{fact.label}</dt><dd>{fact.value}</dd></div>)}</dl></section>)}</div><div className="approved-definition-actions"><button className="solution-primary" onClick={next}>Continue to Data &amp; Knowledge <span>→</span></button></div></WorkspacePage>
}

function Overview({begin}:{begin:()=>void}) {
  const record=activeUseCase
  if(!record)return null
  const currentStage=record.lifecycle.find(stage=>stage.key===record.currentStage)
  return <>
    <section className="solution-hero">
      <div><span className="solution-status">✓ {record.status.toUpperCase()}</span><p>{record.id}</p><h1>{record.name}</h1><p className="hero-copy">{record.summary}</p></div>
    ...
    <div className="solution-layout">
      <div className="solution-main">
        ...
        <section className="solution-card next-stage">
          <div className="section-kicker">CURRENT STAGE · {currentStage?.owner.toUpperCase()}</div><h2>{currentStage?.label??'Complete your solution profile'}</h2><p>Your approved business need and AI capabilities are already included. Now confirm the data, tools, memory, and operating details needed to generate an approved Azure architecture.</p>
...
function ConnectorCatalog({ resources, selected, requested, toggle, back, next:_next }: { resources:Resource[]; selected:string[]; requested:string[]; toggle:(asset:CatalogAsset)=>void; back:()=>void; next:()=>void }) {
  void _next
...
function ArchitectureNeeds({resources,requirements,setRequirement,back,next}:{resources:Resource[];requirements:Requirements;setRequirement:<K extends keyof Requirements>(key:K,value:Requirements[K])=>void;back:()=>void;next:()=>void}) {
  ...
  return <div className={`operational-layout operational-layout-step-${step}`}>
    <section className={`solution-card capability-panel operational-form wizard-step-${step}`}>
      <nav className="operational-stepper" aria-label="Operational requirements progress">
        {(['Operating needs','Review','Outcome'] as const).map((label,index)=>{const number=(index+1) as 1|2|3;return <button type="button" className={step===number?'current':step>number?'complete':''} onClick={()=>setStep(number)} aria-current={step===number?'step':undefined} key={label}><i>{step>number?'✓':number}</i><span>{label}</span></button>})}
      </nav>
      ...
      {step===1&&<div className={`compact-architecture-outcome ${plan?.deployable?'ready':'attention'}`} aria-live="polite"><i>{!plan?'…':plan.deployable?'✓':'!'}</i><span><small>RECOMMENDED PROFILE</small><strong>{plan?.label??'Deriving recommendation…'}</strong></span>{plan&&<div><span><small>RESTORE</small><b>{plan.targets.rtoMinutes===1440?'Next day':`${plan.targets.rtoMinutes} min`}</b></span><span><small>DATA LOSS</small><b>{plan.targets.rpoMinutes===0?'Near zero':`${plan.targets.rpoMinutes} min`}</b></span></div>}<em>{plan&&!plan.deployable?'Action required':'Full details after Review'}</em></div>}
      <section className="operational-section"><header><b>1</b><span><h2>What environments and usage should we plan for?</h2><p>These answers determine deployment structure, capacity, and cost.</p></span></header><div className="inherited-governance"><span><b>Inherited platform standard</b><strong>Private J&amp;J network with managed identity, private endpoints, and approved corporate access paths</strong></span><em>Always applied</em></div><div className="requirement-fields"><label>Lifecycle<select value={requirements.environments} onChange={e=>setRequirement('environments',e.target.value as Requirements['environments'])}><option value="dev">Development only</option><option value="dev-test">Development + test</option><option value="dev-test-prod">Development + test + production</option></select></label><label>Expected usage<select value={requirements.scale} onChange={e=>setRequirement('scale',e.target.value as Requirements['scale'])}><option value="small">Small · under 100 users</option><option value="medium">Medium · 100–5,000 users</option><option value="large">Large · over 5,000 users</option></select></label></div></section>
      ...
      <div className="blueprint-actions"><button className="solution-secondary" onClick={back}>← Data &amp; tools</button><span>{plan?.deployable?'Requirements complete':plan?'Review the recommendation':'Updating recommendation…'}</span><button className="solution-primary" disabled={!plan?.deployable} onClick={next}>Continue to Model &amp; Capacity →</button></div>
      <section className="operational-section operational-review"><header><b>2</b><span><h2>Review your operating expectations</h2><p>Confirm these choices before continuing.</p></span></header><div className="operational-review-grid"><article><span><strong>Environment &amp; expected usage</strong><button onClick={()=>setStep(1)}>Edit</button></span><p>{{dev:'Development only','dev-test':'Development + test','dev-test-prod':'Development + test + production'}[requirements.environments]} · {requirements.scale} scale · Private J&amp;J network standard applied</p></article><article><span><strong>Service criticality</strong><button onClick={()=>setStep(1)}>Edit</button></span><p>{requirements.serviceHours.replace(/-/g,' ')} · RTO {requirements.rtoMinutes===1440?'next business day':`${requirements.rtoMinutes} min`} · RPO {requirements.rpoMinutes===0?'near zero':`${requirements.rpoMinutes} min`}</p></article></div></section>
      <section className="operational-section architecture-outcome-page"><ArchitectureOutcomeDetails plan={plan} planError={planError}/></section><div className="operational-wizard-actions"><button className="solution-secondary" onClick={()=>step===1?back():setStep(previous=>(previous-1) as 1|2|3)}>← {step===1?'Data & tools':'Back'}</button><span>Step {step} of 3</span>{step===1?<button className="solution-primary" onClick={()=>setStep(2)}>Continue to review →</button>:step===2?<button className="solution-primary" onClick={()=>setStep(3)}>See recommended architecture →</button>:<button className="solution-primary" disabled={!plan?.deployable} onClick={next}>Continue to Model &amp; Capacity →</button>}</div>
    </section>
...
type InferredAzureResource={name:string;type:string;purpose:string;reason:string;required:boolean}
export default function SolutionBuilder({applicationId,exit,requestedScreen,onScreenChange,onResourcesChange}:{applicationId:string;exit:()=>void;requestedScreen?:WorkspaceScreen;onScreenChange?:(screen:WorkspaceScreen)=>void;onResourcesChange?:(resources:InferredAzureResource[])=>void}) {
  const [screen, setScreen] = useState<WorkspaceScreen>(restoredWorkspaceScreen)
  const externallyRequestedScreen=useRef<WorkspaceScreen>()
  ...
  useEffect(()=>{
    let active=true
    setRecordError('')
    void getUseCase(applicationId).then(result=>{if(active)setRecord(result)}).catch(error=>{if(active)setRecordError(error instanceof Error?error.message:'Approved application could not be loaded.')})
    return()=>{active=false}
  },[applicationId,recordAttempt])
  ...
  useEffect(()=>{if(!requestedScreen)return;if(requestedScreen===screen){externallyRequestedScreen.current=undefined;return}externallyRequestedScreen.current=requestedScreen;setScreen(requestedScreen)},[requestedScreen])
  useEffect(()=>{if(externallyRequestedScreen.current){if(externallyRequestedScreen.current===screen)externallyRequestedScreen.current=undefined;return}onScreenChange?.(screen)},[onScreenChange,screen])
  useEffect(()=>{onResourcesChange?.(resources.map(({name,type,purpose,reason,required})=>({name,type,purpose,reason,required})))},[onResourcesChange,resources.map(resource=>`${resource.name}:${resource.type}:${resource.required}`).join('|')])
  ...
  const approveArchitecture=async()=>{
    ...
    try{
      ...
      const request:ArchitecturePackageRequest={provisioningMode:'existing-resources',applicationId:record?.id??applicationId,location:platform.location,workloadName:'quality-rag-poc',resources:pocDeploymentKeys,chatModel:chatDeployment,embeddingModel:embeddingDeployment,cosmosPartitionKeyPath,operationalRequirements:operationalRequirements(requirements),tags:{applicationId:record?.id??applicationId,environment:'poc',provisioningMode:'existing-resources',expiresOn:'2026-09-30'}}
  ...
  if(recordError)return <main className="configuration-ready"><div>!</div><span>APPLICATION UNAVAILABLE</span><h1>Approved record could not be loaded</h1><p>{recordError}</p><button className="solution-primary" onClick={()=>setRecordAttempt(attempt=>attempt+1)}>Try again</button><button className="solution-secondary" onClick={exit}>Back to applications</button></main>
  if(!record)return <main className="configuration-ready"><span>LOADING APPROVED RECORD</span><h1>Preparing application workspace</h1></main>
  activeUseCase=record
  ...
  if(screen==='channels')return <div className="solution-shell"><div className="solution-top"><button onClick={exit}>← My applications</button><span>Quality Event Summarizer <b>AI-2026-0118</b></span><button className="solution-secondary">Save workspace</button></div><div className="solution-workspace"><WorkspaceNav screen={screen} resources={resources} profile={profile} navigate={setScreen}/><main className="solution-page workspace-content"><ChannelsDistribution resources={resources} channels={channelConfiguration} setChannels={setChannelConfiguration}/></main></div></div>
  if(String(screen)==='requirements')return <div className="solution-shell"><div className="solution-top"><button onClick={exit}>← My applications</button><span>Quality Event Summarizer <b>AI-2026-0118</b></span><button className="solution-secondary">Save workspace</button></div><div className="solution-workspace"><WorkspaceNav screen={screen} resources={resources} profile={profile} navigate={setScreen}/><main className="solution-page workspace-content"><ArchitectureNeeds resources={resources} requirements={requirements} setRequirement={setRequirement} back={()=>setScreen('memory')} next={()=>setScreen('model')}/></main></div></div>
  if(String(screen)==='model')return <div className="solution-shell"><div className="solution-top"><button onClick={exit}>← My applications</button><span>Quality Event Summarizer <b>AI-2026-0118</b></span><button className="solution-secondary">Save workspace</button></div><div className="solution-workspace"><WorkspaceNav screen={screen} resources={resources} profile={profile} navigate={setScreen}/><main className="solution-page workspace-content"><ModelSelection resources={resources} recommendation={modelRecommendation} selected={selectedModel.id} setSelected={setModelOverride} requiresEmbedding={requiresEmbedding} selectedEmbedding={embeddingModel} setSelectedEmbedding={setEmbeddingModel} embeddingCapacity={embeddingCapacity} setEmbeddingCapacity={setEmbeddingCapacity} next={()=>{setModelConfirmed(true);setScreen('region')}}/></main></div></div>
  ...
  if(screen==='definition')return <div className="solution-shell"><div className="solution-top"><button onClick={exit}>← My applications</button><span>Quality Event Summarizer <b>AI-2026-0118</b></span><button className="solution-secondary">Approved record</button></div><div className="solution-workspace"><WorkspaceNav screen={screen} resources={resources} profile={profile} navigate={setScreen}/><main className="solution-page workspace-content"><ApprovedDefinition next={()=>setScreen('data')}/></main></div></div>
  return <div className="solution-shell"><div className="solution-top"><button onClick={exit}>← My applications</button><span>Quality Event Summarizer <b>AI-2026-0118</b></span><button className="solution-secondary">Save workspace</button></div><div className="solution-workspace"><WorkspaceNav screen={screen} resources={resources} profile={profile} navigate={setScreen}/><main className="solution-page workspace-content">{screen === 'overview' && <Overview begin={() => setScreen('design')}/>} {screen==='design'&&<DesignStart describe={()=>setScreen('describe')} guided={()=>setScreen('data')}/>} {screen==='describe'&&<DescribeApplication back={()=>setScreen('design')} apply={(caps,data,memory,inferredRequirements)=>{setSelected(caps);setDataConfiguration(data);setMemorySelections(memory);setRequirements(current=>({...current,...inferredRequirements}));setScreen('data')}}/>} {screen==='data'&&<DataKnowledge resources={resources} data={dataConfiguration} setData={setDataConfiguration}/>} {screen === 'connectors' && <ConnectorCatalog resources={resources} selected={selectedAssets} requested={requestedAssets} toggle={toggleConnector} back={() => setScreen('data')} next={() => setScreen('memory')}/>} {screen==='memory'&&<MemoryState resources={resources} memory={memorySelections} setMemory={setMemorySelections}/>} {screen === 'requirements' && <ArchitectureNeeds resources={resources} requirements={requirements} setRequirement={setRequirement} back={() => setScreen('memory')} next={() => setScreen('architecture')}/>} {screen === 'architecture' && <Architecture resources={resources} requirements={requirements} addedResources={addedResources} cosmosPartitionKeyPath={cosmosPartitionKeyPath} setCosmosPartitionKeyPath={setCosmosPartitionKeyPath} addResource={name=>{setAddedResources(current=>[...new Set([...current,name])]);setRemovedResources(current=>{const next={...current};delete next[name];return next})}} removeResource={(name,reason)=>{setAddedResources(current=>current.filter(item=>item!==name));setRemovedResources(current=>({...current,[name]:reason}))}} back={() => setScreen('requirements')} next={() => setScreen('iac')}/>} {screen==='iac'&&<InfrastructureCode resources={resources}/>} {screen==='governance'&&<GovernancePage requirements={requirements} setRequirement={setRequirement}/>} {screen==='provisioning'&&<ProvisioningPage resources={resources}/>} {screen==='activity'&&<ActivityPage/>}</main></div>{requestAsset&&<AssetRequestModal asset={requestAsset} close={()=>setRequestAsset(null)} submit={submitAssetRequest}/>}</div>
}
